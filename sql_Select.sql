...
FROM city
WHERE CountryCode = "TJK" OR "MRT" OR "AND" OR "PNG" OR "SJM";
...
SELECT Name AS COUNTRIES, Population
FROM country 
WHERE Population BETWEEN 8000 AND 20000;

# 16: Get the name of the countries with a independence year (indepyear) before year 0

SELECT Name AS COUNTRIES, IndepYear
FROM country 
WHERE IndepYear < 0 ;

# 17: Get the countries that has no recorded independence year and a population above 1000000

SELECT Name AS COUNTRIES, IndepYear, Population
FROM country 
WHERE IndepYear IS NULL AND Population > 1000000;

# 18: Get countries with a SurfaceArea below 10 and a defined LifeExpectancy

SELECT Name AS COUNTRIES, SurfaceArea, LifeExpectancy
FROM country 
